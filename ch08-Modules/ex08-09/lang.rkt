...
(define the-grammar
  '(
    (program ((arbno module-definition) (arbno dependencies) expression) a-program)
    ...
    (module-body ((arbno dependencies) "[" (arbno definition) "]") defns-module-body)
    (module-body
      (
    ...
    (dependencies ("depends-on" identifier (arbno "," identifier)) module-dependencies)
...
(define type-to-external-form
  (lambda (ty)
    (cases type ty
      ...
      (iface-type (iface) (list 'iface iface))
    )
  )
)
...
; Listof(ModuleDeps) -> Listof(ModuleDep)
(define extract-m-deps
  (lambda (m-depss)
    (if (null? m-depss)
      '()
      (cases dependencies (car m-depss)
        (module-dependencies (m ms)
          (append (cons m ms) (extract-m-deps (cdr m-depss)))
        )
      )
    )
  )
...
; Symbol * Listof(ModuleDep) -> ()
(define report-module-not-found-error
  (lambda (m m-deps)
    (eopl:error 'module-not-found-error "module '~s' not found in dependencies ~s." m m-deps)
  )
)
