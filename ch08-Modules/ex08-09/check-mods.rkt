...
(define type-of-program
  (lambda (pgm)
    (cases program pgm
      (a-program (module-defns module-depss body)
        (type-of body
          (add-module-defns-to-tenv module-defns (empty-tenv))
          (extract-m-deps module-depss)
        )
      )
    )
  )
...
(define add-module-defns-to-tenv
  (lambda (module-defns tenv)
    (if (null? module-defns)
      tenv
      (cases module-definition (car module-defns)
        (a-module-definition (m-name expected-iface m-body)
          (if (variable-name->maybe-binding-in-tenv tenv m-name)
            (eopl:error 'module-name-dupulicated "module named ~s is already defined" m-name)
            (let ([actual-iface (interface-of m-body tenv)])
              (if (<:-iface actual-iface expected-iface tenv)
                (let ([new-tenv (extend-tenv m-name (iface-type expected-iface) tenv)])
                  (add-module-defns-to-tenv (cdr module-defns) new-tenv)
                )
  ...
  (lambda (m-body tenv)
    (cases module-body m-body
      (defns-module-body (depss defns)
        (simple-iface (defns-to-decls defns tenv (extract-m-deps depss)))
      )
      ...
      (a-module-body-with-local-modules (module-defn module-defns defns)
        (simple-iface
          (defns-to-decls defns
            (add-module-defns-to-tenv (cons module-defn module-defns) tenv)
            '()
          )
        )
      )
...
; Listof(Defn) * Tenv * Listof(ModuleDep) -> Listof(Decl)
(define defns-to-decls
  (lambda (defns tenv m-deps)
    (if (null? defns)
      '()
      (cases definition (car defns)
        (val-defn (var-name exp)
          (let ([ty (type-of exp tenv m-deps)])
            (let ([new-tenv (extend-tenv var-name ty tenv)])
              (cons
                (val-decl var-name ty)
                (defns-to-decls (cdr defns) new-tenv m-deps)
              )
            )
          )
        )
