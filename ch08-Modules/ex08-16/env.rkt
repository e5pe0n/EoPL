  ...
  (lambda (env search-var)
    (cases environment env
      ...
      (extend-env* (saved-vars saved-vals saved-env)
        (let ([res (position (lambda (x) (eqv? x search-var)) saved-vars)])
          (if res
            (list-ref saved-vals res)
            (apply-env saved-env search-var)
          )
        )
      )
      ...
      (extend-env-rec* (p-names b-varss p-bodys saved-env)
        (let* (
            [ps (zip p-names b-varss p-bodys)]
            [res (position (lambda (x) (eqv? (car x) search-var)) ps)]
          )
          (if res
            (let* (
                [p (list-ref ps res)]
