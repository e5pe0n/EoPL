...
(define expval->pair
  (lambda (val)
    (cases expval val
      (pair-val (pair1) pair1)
      (else report-expval-extractor-error 'expval->pair val)
    )
  )
)
...
(define expval->list
  (lambda (val)
    (cases expval val
      ...
      (else report-expval-extractor-error 'expval->list val)
    )
  )
)
...
(define expval->any
  (lambda (val)
    (cases expval val
      ...
      (pair-val (pair1) pair1)
      (list-val (list1) list1)
      (else report-expval-extractor-error 'expval->any val)
    )
  )
)
...
(define any->expval
  (lambda (x)
    (cond
      ...
      ((procedure? x) (proc-val x))
      ((pair? x) (pair-val x))
      ((list? x) (list-val x))
      (else report-invalid-scheme-val-error x)
    )
  )
)
