  ...
  (lambda (pgm)
    (cases program pgm
      (a-program (exp1)
        (begin
          (init-sn!)
          (init-apply-subst-to-type-cache!)
...
; Pairof(Tvar, Int)
(define apply-subst-to-type-cache 'uninitialized)

(define init-apply-subst-to-type-cache!
  (lambda () (set! apply-subst-to-type-cache '()))
)
...
(define apply-subst-to-type
  (lambda (ty subst)
    (cases type ty
      ...
      (tvar-type (sn)
        (let ([subst-len (length subst)])
          (let ([p (assoc (cons ty subst-len) apply-subst-to-type-cache)])
            (if p
              (cdr p)
              (let loop1 ([subst subst] [r-subst '()])
                (if (null? subst)
                  ty
                  (let* ([p1 (car subst)] [lhs1 (car p1)] [rhs1 (cdr p1)])
                    (if (equal? lhs1 ty)
                      (let loop2 ([r-subst r-subst] [t rhs1])
                        (if (null? r-subst)
                          (begin
                            (set! apply-subst-to-type-cache
                              (cons (cons (cons ty subst-len) t) apply-subst-to-type-cache)
                            )
                            t
                          )
                          (let* ([p2 (car r-subst)] [lhs2 (car p2)] [rhs2 (cdr p2)])
                            (loop2 (cdr r-subst) (apply-one-subst t lhs2 rhs2))
                          )
                        )
                      )
                      (loop1 (cdr subst) (cons p1 r-subst))
                    )
                  )
                )
              )
