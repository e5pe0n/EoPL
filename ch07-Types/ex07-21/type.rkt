...
(define apply-subst-to-type
  (lambda (ty)
    (cases type ty
      ...
      (tvar-type (sn)
        (let loop1 ([subst the-subst] [r-subst '()])
          (if (null? subst)
            ty
...
(define get-tvar
  (lambda (tvar)
    (let ([p (assoc tvar the-subst)])
      (if p
        (cdr p)
        (report-invalid-tvar tvar the-subst)
      )
    )
  )
)
    ...
    (set! the-subst
      (letrec ([
          ...
          (lambda (subst)
            (if (null? subst)
              (report-invalid-tvar tvar the-subst)
              (let* ([p (car subst)] [lhs (car p)])
                (if (equal? tvar lhs)
                  (cons (cons lhs ty) subst)
                  (cons p (set-tvar-inner (cdr subst)))
                )
              )
            )
          )
        ])
        (set-tvar-inner the-subst)
      )
    )
  )
)
