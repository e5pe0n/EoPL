  ...
  (lambda (pgm)
    (cases program pgm
      (a-program (exp1)
        (begin
          (init-sn!)
          (cases answer (type-of exp1 (init-tenv) (empty-eqs))
            (an-answer (ty eqs)
...
; SchemeVal -> Bool
(define equations?
  (list-of (lambda (x) (and (type? (car x)) (type? (cadr x)) (expression? (caddr x)))))
)
...
; Tvar * Type * Exp * Eqs -> Eqs
(define extend-eqs
  (lambda (tvar ty exp eqs)
    (cons (list tvar ty exp) eqs)
  )
)
...
(define type-of
  (lambda (exp tenv eqs)
    (cases expression exp
      ...
      (var-exp (var) (an-answer (apply-tenv tenv var) eqs))
      (diff-exp (exp1 exp2)
        (cases answer (type-of exp1 tenv eqs)
          (an-answer (ty1 eqs)
            (let ([eqs (extend-eqs ty1 (int-type) exp1 eqs)])
              (cases answer (type-of exp2 tenv eqs)
                (an-answer (ty2 eqs)
                  (let ([eqs (extend-eqs ty2 (int-type) exp2 eqs)])
      ...
      (if-exp (exp1 exp2 exp3)
        (cases answer (type-of exp1 tenv eqs)
          (an-answer (ty1 eqs)
            (let ([eqs (extend-eqs ty1 (bool-type) exp1 eqs)])
              (cases answer (type-of exp2 tenv eqs)
                (an-answer (ty2 eqs)
                  (cases answer (type-of exp3 tenv eqs)
                    (an-answer (ty3 eqs)
                      (let ([eqs (extend-eqs ty2 ty3 exp eqs)])
                        (an-answer ty2 eqs)
                      )
                    )
                  )
      ...
      (let-exp (var exp1 body)
        (cases answer (type-of exp1 tenv eqs)
          (an-answer (exp1-type eqs)
            (type-of body (extend-tenv var exp1-type tenv) eqs)
          )
        )
      )
      (proc-exp (var otype body)
        (let ([var-type (otype->type otype)])
          (cases answer (type-of body (extend-tenv var var-type tenv) eqs)
            (an-answer (body-type eqs)
              (an-answer (proc-type var-type body-type) eqs)
            )
          )
        )
      )
            ...
            (an-answer (rator-type eqs)
              (cases answer (type-of rand tenv eqs)
                (an-answer (rand-type eqs)
                  (let ([eqs (extend-eqs rator-type (proc-type rand-type result-type) rator eqs)])
      ...
      (letrec-exp (p-result-otype p-name b-var b-var-otype p-body letrec-body)
        (let (
          ...
          (let ([tenv-for-letrec-body (extend-tenv p-name (proc-type b-var-type p-result-type) tenv)])
            (cases answer (type-of p-body (extend-tenv b-var b-var-type tenv-for-letrec-body) eqs)
              (an-answer (p-body-type eqs)
                (let ([eqs (extend-eqs p-body-type p-result-type p-body eqs)])
                  (type-of letrec-body tenv-for-letrec-body eqs)
                )
              )
            )
          )
...
(define unify
  (lambda (eqs)
    (reduce
      (lambda (acc v) (unifier (car v) (cadr v) acc (caddr v)))
      eqs (empty-subst)
    )
  )
)
