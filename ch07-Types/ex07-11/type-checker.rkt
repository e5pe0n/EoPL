...
;   left(1)
; "))
; ; left-exp: expression must be a mutpair type;
; ;   received type=int
; ;   received expression=#(struct:const-exp 1)

(println (type-check! "
  let p = pair(11, 22)
  in setleft(p, zero?(left(p)))
"))
; check-equal-type!: Types didn't match: int != bool in
; #(struct:setleft-exp #(struct:var-exp p) #(struct:zero?-exp #(struct:left-exp #(struct:var-exp p))))
