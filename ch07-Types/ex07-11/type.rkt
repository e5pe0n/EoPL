  ...
  (lambda (exp tenv)
    (cases expression exp
      ...
      (newpair-exp (exp1 exp2)
        (mutpair-type (type-of exp1 tenv) (type-of exp2 tenv))
      )
      (left-exp (exp1)
        (let ([ty (type-of exp1 tenv)])
          (cases type ty
            (mutpair-type (ty1 ty2) ty1)
            (else (report-exp-not-a-mutpair-type 'left-exp ty exp1))
          )
        )
      )
      (right-exp (exp1)
        (let ([ty (type-of exp1 tenv)])
          (cases type ty
            (mutpair-type (ty1 ty2) ty1)
            (else (report-exp-not-a-mutpair-type 'right-exp ty exp1))
          )
        )
      )
      (setleft-exp (exp1 exp2)
        (let ([ty1 (type-of exp1 tenv)])
          (cases type ty1
            (mutpair-type (t1 t2)
              (check-equal-type! t1 (type-of exp2 tenv) exp)
            )
            (else (report-exp-not-a-mutpair-type 'setleft-exp ty1 exp1))
          )
        )
      )
      (setright-exp (exp1 exp2)
        (let ([ty1 (type-of exp1 tenv)])
          (cases type ty1
            (mutpair-type (t1 t2)
              (check-equal-type! t2 (type-of exp2 tenv) exp)
            )
            (else (report-exp-not-a-mutpair-type 'setright-exp ty1 exp1))
          )
        )
      )
    )
...
; Symbol * Type * Exp -> Unspecified
(define report-exp-not-a-mutpair-type
  (lambda (err-name ty exp)
    (eopl:error err-name
      "expression must be a mutpair type;~%  received type=~s~%  received expression=~s"
  ...
  (lambda (ty)
    (cases type ty
      ...
      (mutpair-type (ty1 ty2)
        (list 'mutpair (type-to-external-form ty1) (type-to-external-form ty2))
      )
    )
  )
)
