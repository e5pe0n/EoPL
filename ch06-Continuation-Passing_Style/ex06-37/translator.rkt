  ...
  (lambda (exp k-exp)
    (cases expression exp
      ...
      (let-exp (vars rhss body)
        (cps-of-exps rhss
          (lambda (simples)
            (let f ([vs vars] [ss simples])
              (if (null? vs)
                (cps-of-exp body k-exp)
                (cps-newrefk-exp (car ss)
                  (cps-proc-exp (list (car vs))
                    (f (cdr vs) (cdr ss))
                  )
                )
              )
      ...
      (letrec-exp (p-names b-varss p-bodies letrec-body)
        (let f ([names p-names])
          (if (null? names)
            (let g ([ns p-names] [varss b-varss] [bodies p-bodies])
              (if (null? ns)
                (cps-of-exp letrec-body k-exp)
                (cps-setrefk-exp (cps-var-exp (car ns))
                  (cps-proc-exp
                    (append (car varss) (list 'k%00))
                    (cps-of-exp (car bodies) (cps-var-exp 'k%00))
                  )
                  (g (cdr ns) (cdr varss) (cdr bodies))
                )
              )
            )
            (cps-newrefk-exp
      ...
      (call-exp (rator rands)
        (cps-of-exps (cons rator rands)
          (lambda (simples)
            (let f ([ss (cdr simples)] [ref-vars '()])
              (if (null? ss)
                (cps-call-exp (car simples) (append ref-vars (list k-exp)))
                (let ([ref-var (fresh-identifier 'var)])
                  (cps-newrefk-exp (car ss)
                    (cps-proc-exp (list ref-var)
                      (f (cdr ss) (append ref-vars (list (cps-var-exp ref-var))))
                    )
                  )
                )
              )
      ...
      (begin-exp (exps)
        (let f ([es exps])
          (if (= (length es) 1)
            (cps-of-exp (car es) k-exp)
            (cps-of-exp (car es)
              (cps-proc-exp (list (fresh-identifier 'var))
                (f (cdr es))
              )
            )
          )
      ...
      (assign-exp (var exp1)
        (cps-of-exps (list exp1)
          (lambda (simples)
            (cps-setrefk-exp (cps-var-exp var) (car simples)
