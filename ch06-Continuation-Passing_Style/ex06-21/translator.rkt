...
(define cps-of-exps
  (lambda (exps builder)
    ...
    (let cps-of-rest ([exps exps])
      (let
        ([pos (list-last-index (lambda (exp) (not (inp-exp-simple? exp))) exps)])
        (if (not pos)
    ...
    (cases expression exp
      ...
      (call-exp (rator rands)
        (cps-of-exps (append rands (list rator))
          (lambda (simples)
            (let loop ([ss simples] [rds '()])
              (if (= (length ss) 1)
                (cps-call-exp (car ss) (append rds (list k-exp)))
                (loop (cdr ss) (append rds (list (car ss))))
              )
            )
          )
        )
