    ...
    (cases expression exp
      ...
      (proc-exp (vars body)
        (make-send-to-cont k-exp
          (cps-proc-exp (append vars (list 'k%00 'exc-k%00))
            (cps-of-exp body (cps-var-exp 'k%00) (cps-var-exp 'exc-k%00))
          )
        )
      )
      (zero?-exp (exp1)
        ...
        (cps-of-exps (list exp1)
          (lambda (simples)
            (cps-if-exp (car simples)
              (cps-of-exp exp2 k-exp exc-k-exp)
              (cps-of-exp exp3 k-exp exc-k-exp)
            )
          )
        )
      )
      (let-exp (vars rhss body)
        (cps-of-exps rhss
          (lambda (simples)
            (cps-let-exp vars simples (cps-of-exp body k-exp exc-k-exp))
          )
        )
      )
      (letrec-exp (p-names b-varss p-bodies letrec-body)
        (cps-letrec-exp p-names
          (map
            (lambda (b-vars)
              (append b-vars (list 'k%00 'exc-k%00))
            )
            b-varss
          )
          (map
            (lambda (p-body)
              (cps-of-exp p-body (cps-var-exp 'k%00) (cps-var-exp 'exc-k%00))
            )
            p-bodies
          )
          (cps-of-exp letrec-body k-exp exc-k-exp)
        )
      )
      (call-exp (rator rands)
        (cps-of-exps (cons rator rands)
          (lambda (simples)
            (cps-call-exp (car simples) (append (cdr simples) (list k-exp exc-k-exp)))
          )
        )
      )
      (emptylist-exp ()
      ...
      (try-exp (exp1 var exp2)
        (cps-of-exp exp1 k-exp
          (cps-proc-exp (list var)
            (cps-of-exps (list exp2)
              (lambda (simples)
                (make-send-to-cont exc-k-exp (car simples))
              )
            )
          )
        )
...
(define cps-of-simple-exp
  (lambda (exp)
    (cases expression exp
      ...
      (proc-exp (vars exp1)
        (cps-proc-exp
          (append vars (list 'k%00 'exc-k%00))
          (cps-of-exp exp1 (cps-var-exp 'k%00) (cps-var-exp 'exc-k%00))
        )
      )
      (add1-exp (exp1)
