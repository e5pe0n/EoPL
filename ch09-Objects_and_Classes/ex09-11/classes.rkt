  ...
  (lambda (obj field-name)
    (let*
      (
        ...
        [pos (position (lambda (x) (eqv? x field-name)) (class->field-names cls))]
      )
      (if pos
        (list-ref (object->fields obj) pos)
        (eopl:error 'fieldref "no such field ~s in ~s" field-name (object->class-name obj))
      )
    )
  )
)
