                ...
                (cond
                  [
                    ...
                    (let ([maybe-self (find-env env '%self)])
                      (if (and maybe-self (eqv? (object->class-name maybe-self) class-name))
                        (apply-method m obj args)
                        (report-not-allowed-method-call class-name method-name)
                      )
                    )
                  ]
                  [
                  ...
                  [(eqv? modifier 'public) (apply-method m obj args)]
                )
              )
            )
          )
      ...
      (superfieldref-exp (exp1 field-name)
        (let*
          (
            ...
            [cls (lookup-class (object->class-name obj))]
            [super-name (class->super-name cls)]
          )
          (deref (fieldref obj (mangle-field-name super-name field-name)))
        )
      )
      (superfieldset-exp (exp1 field-name exp2)
        (let ([obj (value-of exp1 env)] [v2 (value-of exp2 env)])
          (let*
            ...
            (let ([ref (fieldref obj (mangle-field-name super-name field-name))])
              (setref! ref v2)
            )
          )
        )
