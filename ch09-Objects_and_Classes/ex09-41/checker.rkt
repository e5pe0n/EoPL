  ...
  (lambda (exp tenv)
    (cases expression exp
      ...
      (fieldref-exp (obj-exp field-name)
        (let* ([obj-type (type-of obj-exp tenv)] [sc (lookup-static-class (type->class-name obj-type))])
          (list-ref
            (static-class->field-types sc)
            (position (lambda (x) (eqv? x field-name)) (static-class->field-names sc))
          )
        )
      )
      (fieldset-exp (obj-exp field-name exp1)
        (let* ([obj-type (type-of obj-exp tenv)] [sc (lookup-static-class (type->class-name obj-type))])
          (let (
            ...
            (check-is-subtype! (type-of exp1 tenv) field-type exp)
            (void-type)
          )
        )
      )
