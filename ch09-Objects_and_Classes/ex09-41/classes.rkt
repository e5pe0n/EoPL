...
(define fieldref
  (lambda (obj field-name)
    (let ([cls (lookup-class (object->class-name obj))])
      (let ([pos (position (lambda (x) (eqv? x field-name)) (class->field-names cls))])
        (if pos
          (list-ref (object->fields obj) pos)
          (eopl:error 'fieldref "field ~s does not exist in class ~s." field-name (object->class-name obj))
        )
      )
    )
  )
