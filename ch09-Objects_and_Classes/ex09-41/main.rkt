...
(println (run "
  ...
  in begin
    fieldset o1 x = 100;
    -(0, fieldref o1 x)   % #(struct:num-val -100)
    % ;fieldset o1 x = zero?(1)
    %   check-is-subtype!: bool is not a subtype of int in
    %   #(struct:fieldset-exp #(struct:var-exp o1) x #(struct:zero?-exp #(struct:const-exp 1)))
  end
"))
