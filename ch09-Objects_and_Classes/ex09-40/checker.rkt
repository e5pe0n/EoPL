...
(define check-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
      ...
      (a-class-decl (class-name super-name i-names f-types f-names f-exps method-decls)
        ...
        (for-each
          (lambda (f-type f-name f-exp)
            (check-field-type! f-type f-name f-exp)
          )
          f-types f-names f-exps
        )
      )
    )
  )
)
(define check-field-type!
  (lambda (f-type f-name f-exp)
    (check-is-subtype!
      (type-of f-exp (init-tenv)) f-type
      f-exp
    )
  )
)
