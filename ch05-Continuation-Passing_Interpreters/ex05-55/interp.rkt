...
(define the-mutex-wait-queue-refs '())
...
(define new-mutex
  (lambda ()
    (let ([ref (newref '())])
      (set! the-mutex-wait-queue-refs (cons ref the-mutex-wait-queue-refs))
      (a-mutex (newref #f) ref)
    )
  )
)
...
(define kill-th!
  (lambda (th-id)
    (or
      ...
      (let f ([refs the-mutex-wait-queue-refs] [killed #f])
        (if (null? refs)
          killed
          (let* ([ref (car refs)] [q (deref ref)] [org-len (length q)] [res-q (filter q (lambda (th) (match-th th th-id)))])
            (setref! ref res-q)
            (f (cdr refs) (or killed (not (= org-len (length res-q)))))
          )
        )
      )
    )
        ...
        (cases continuation cont
          ...
          (wait-cont (saved-cont)
            (wait-for-mutex
              ...
              (a-thread
                (lambda (th-id1 parent-th-id1 msg-queue1 cont1)
                  (apply-cont cont1 (num-val 52) th-id1 parent-th-id1 msg-queue1)
                )
                th-id parent-th-id msg-queue saved-cont
              )
            )
          )
          (signal-cont (saved-cont)
            (signal-mutex
              ...
              (a-thread
                (lambda (th-id1 parent-th-id1 msg-queue1 cont1)
                  (apply-cont cont1 (num-val 53) th-id1 parent-th-id1 msg-queue1)
                )
                th-id parent-th-id msg-queue saved-cont
              )
            )
...
(define value-of/k
  (lambda (exp env cont th-id parent-th-id msg-queue)
    (cases expression exp
      ...
      (var-exp (var)
        (apply-cont cont (deref (apply-env env var)) th-id parent-th-id msg-queue)
      )
      (proc-exp (vars body)
        (apply-cont cont
      ...
      (mutex-exp ()
        (apply-cont cont (mutex-val (new-mutex)) th-id parent-th-id msg-queue)
      )
      (wait-exp (exp1)
        (value-of/k exp1 env (wait-cont cont) th-id parent-th-id msg-queue)
      )
      (signal-exp (exp1)
        (value-of/k exp1 env (signal-cont cont) th-id parent-th-id msg-queue)
      )
      (yield-exp ()
        (begin
          (place-on-ready-queue!
