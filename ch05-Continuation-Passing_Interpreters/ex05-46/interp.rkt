...
; Thread = () -> ExpVal
; Timer = Int
; Queue = Listof(Pairof(Thread, Timer))
...
; Queue * Pairof(Thread, Timer) -> Queue
(define enqueue
  (lambda (q th-and-timer) (append q (list th-and-timer)))
)

; Queue * f(Pairof(Thread, Timer), Queue) -> ExpVal
(define dequeue
...
; Pairof(Thread, Timer) -> Unspecified
(define place-on-ready-queue!
  (lambda (th-and-timer)
    (set! the-ready-queue (enqueue the-ready-queue th-and-timer))
  )
)
  ...
  (lambda ()
    (if (empty? the-ready-queue)
      ...
      (dequeue the-ready-queue
        (lambda (head rest)
          (let ([th (car head)] [timer (cdr head)])
            (set! the-ready-queue rest)
            (set! the-time-remaining timer)
            (th)
          )
        )
      )
...
(define apply-cont
  (lambda (cont val)
    (if (time-expired?)
      (begin
        (let ([th (lambda () (apply-cont cont val))])
          (place-on-ready-queue! (cons th the-max-time-slice))
          (run-next-thread)
        )
      )
      (begin
          ...
          (spawn-cont (saved-cont)
            (let ([proc1 (expval->proc val)])
              (place-on-ready-queue!
                (let ([th (lambda () (apply-procedure/k proc1 '(num-val 28) (end-subthread-cont)))])
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ...
      (yield-exp ()
        (begin
          (let ([th (lambda () (apply-cont cont (num-val 99)))])
            (place-on-ready-queue! (cons th the-time-remaining))
            (run-next-thread)
          )
        )
      )
