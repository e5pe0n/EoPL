  ...
  (lambda (cont val)
    (cases continuation cont
      ...
      (div2-cont (val1 saved-cont)
        (let ([num1 (expval->num val1)] [num2 (expval->num val)])
          (if (zero? num2)
            (apply-cont (raise1-cont (div2-cont val1 saved-cont)) (num-val 101))
            (apply-cont saved-cont
              (num-val (/ num1 num2))
            )
          )
          ...
          (cases proc proc1
            (procedure (vars body p-saved-env)
              (if (not (= (length vars) (length rands)))
                (apply-cont (raise1-cont (rator-cont rands saved-env saved-cont)) (num-val 100))
                (if (null? rands)
...
; ExpVal * Cont * Cont -> FinalAnswer
(define apply-handler
  (lambda (val cont raised-cont)
    (cases continuation cont
      (try-cont (var handler-exp saved-env saved-cont)
      ...
      (zero1-cont (saved-cont)
        (apply-handler val saved-cont raised-cont)
      )
      (let-exp-cont (vars exps rvars vals body saved-env saved-cont)
        (apply-handler val saved-cont raised-cont)
      )
