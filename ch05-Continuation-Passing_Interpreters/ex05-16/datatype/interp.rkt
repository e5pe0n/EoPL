...
(define-datatype command-continuation command-continuation?
  (end-cmd-cont)
  (assign-cmd-cont
    (var identifier?)
    (val expval?)
    (saved-env environment?)
    (saved-cmd-cont command-continuation?)
  )
  (print-cmd-cont
    (val expval?)
    (saved-cmd-cont command-continuation?)
...
(define apply-cmd-cont
  (lambda (cmd-cont)
    (cases command-continuation cmd-cont
      (end-cmd-cont ()
        (eopl:printf "End of Computation.~%")
      )
      (assign-cmd-cont (var val saved-env saved-cmd-cont)
        (begin
          (setref! (apply-env saved-env var) val)
          (apply-cmd-cont saved-cmd-cont)
        )
      )
      (print-cmd-cont (val saved-cmd-cont)
        (begin
          (println val)
          (apply-cmd-cont saved-cmd-cont)
        )
      )
      (begin-cmd-cont (stmts saved-env saved-cmd-cont)
        (if (null? stmts)
          (apply-cmd-cont saved-cmd-cont)
          (result-of (car stmts) saved-env (begin-cmd-cont (cdr stmts) saved-env saved-cmd-cont))
        )
      )
      (if-cmd-cont (val stmt1 stmt2 saved-env saved-cmd-cont)
        (if (expval->bool val)
          (result-of stmt1 saved-env saved-cmd-cont)
          (result-of stmt2 saved-env saved-cmd-cont)
        )
      )
      (while-cmd-cont (val exp1 stmt1 saved-env saved-cmd-cont)
        (if (expval->bool val)
