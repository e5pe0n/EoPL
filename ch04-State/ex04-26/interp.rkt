  ...
  (lambda (stmt env)
    (cases statement stmt
      ...
      (var-stmt (vars exps stmt1)
        (let f ([es exps] [b-vars-list '()] [p-bodies '()])
          (if (null? es)
            (result-of stmt1
              (extend-env-rec vars b-vars-list p-bodies env)
            )
            (let ([proc1 (expval->proc (value-of (car es) env))])
              (cases proc proc1
                (procedure (b-vars p-body saved-env)
                  (f (cdr es) (append b-vars-list (list b-vars)) (append p-bodies (list p-body)))
                )
              )
            )
          )
