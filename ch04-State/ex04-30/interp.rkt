...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (newarray-exp (exps)
        (array-val
          (makearray (map (lambda (x) (value-of x env)) exps))
        )
      )
      (arrayref-exp (exp1 exp2)
        (let ([arr1 (expval->arr (value-of exp1 env))] [n (expval->num (value-of exp2 env))])
          (arrayref arr1 n)
        )
      )
      (arrayset-exp (exp1 exp2 exp3)
        (let (
            [arr1 (expval->arr (value-of exp1 env))]
            [n (expval->num (value-of exp2 env))]
          )
          (arrayset arr1 n (value-of exp3 env))
        )
      )
      (arraylength-exp (exp1)
        (let ([arr1 (expval->arr (value-of exp1 env))])
          (num-val
            (arraylength arr1)
          )
        )
