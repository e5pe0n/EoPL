...
(define apply-procedure
  (lambda (proc1 arg)
    (cases proc proc1
      ...
      (trace-procedure (param body saved-env)
        (begin
          (print "enter")
          (let ([v1 (value-of body (extend-env param arg saved-env))])
            (print "exit")
            v1
          )
        )
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (proc-exp (param body)
        (proc-val (procedure param body (mk-frees-env param body env)))
      )
      (traceproc-exp (param body)
        (proc-val (trace-procedure param body (mk-frees-env param body env)))
      )
      (call-exp (rator rand)
        (let
          (
...
(define mk-frees-env
  (lambda (param body env)
    (cases expression body
      ...
      (diff-exp (exp1 exp2)
        (cons-env
          (mk-frees-env param exp1 env)
          (mk-frees-env param exp2 env)
        )
      )
      (add-exp (exp1 exp2)
        (cons-env
      ...
      (if-exp (exp1 exp2 exp3)
        (cons-env
          ...
          (cons-env
            (mk-frees-env param exp2 env)
            (mk-frees-env param exp3 env)
          )
        )
      )
      (var-exp (var)
        (if (and (not (eqv? param var)) (has-binding? env var))
      ...
      (proc-exp (_param _body)
        (empty-env)
      )
      (traceproc-exp (_param _body)
        (empty-env)
      )
      (call-exp (rator rand)
        (cons-env
          (mk-frees-env param rator env)
          (mk-frees-env param rand env)
        )
      )
    )
  )
