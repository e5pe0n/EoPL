...
(define procedure
  (lambda (param body saved-env)
    (lambda (arg)
      (value-of
        body
        (extend-env param (lambda () arg) saved-env)
      )
    )
  )
)
...
(define num-val
  (lambda (num)
    (lambda (type)
      (if (eqv? type 'num)
        num
        (report-expval-extractor-error 'num)
      )
    )
  )
)
(define bool-val
  (lambda (bool)
    (lambda (type)
      (if (eqv? type 'bool)
        bool
        (report-expval-extractor-error 'bool)
      )
    )
  )
)
(define proc-val
  (lambda (proc1)
    (lambda (type)
      (if (eqv? type 'proc)
        proc1
        (report-expval-extractor-error 'proc)
      )
    )
  )
)
...
(define let-exp
  (lambda (var exp1 body)
    (lambda (env)
      (value-of
        body
        (extend-env var (lambda () (value-of exp1 env)) env)
      )
    )
  )
)
  ...
  (lambda (p-name param p-body letrec-body)
    (lambda (env)
      (value-of
        letrec-body
        (letrec
          ([e (extend-env p-name (lambda () (value-of (proc-exp param p-body) e)) env)])
          e
        )
      )
    )
