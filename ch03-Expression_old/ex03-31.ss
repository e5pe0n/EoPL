...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (params exps body)
        (value-of
          body
          (let f ([ps params] [es exps])
            (if (null? ps)
              env
              (extend-env (car ps) (value-of (car es) env) (f (cdr ps) (cdr es)))
            )
          )
        )
      )
      (letrec-exp (p-name params p-body letrec-body)
        (value-of
          letrec-body
          (extend-env-rec p-name params p-body env)
        )
      )
      (minus-exp (exp1)
...
(print
  (expval->num
    (value-of
      (letrec-exp 'mul (list 'x 'y)
        (if-exp (zero?-exp (var-exp 'x))
          (const-exp 0)
          (diff-exp
            (call-exp (var-exp 'mul)
              (list
        ...
        (call-exp (var-exp 'mul) (list (const-exp 3) (const-exp 4)))
      )
      (empty-env)
    )
  )
