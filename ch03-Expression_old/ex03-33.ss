...
(define apply-env
  (lambda (env search-var)
    (cases environment env
      ...
      (extend-env-rec (p-names b-varss p-bodys saved-env)
        (let f ([p-ns p-names] [b-vss b-varss] [p-bs p-bodys])
          (if (null? p-ns)
            (apply-env saved-env search-var)
            (if (eqv? (car p-ns) search-var)
              (proc-val (procedure (car b-vss) (car p-bs) env))
              (f (cdr p-ns) (cdr b-vss) (cdr p-bs))
            )
          )
        )
      )
...
(define-datatype expression expression?
  ...
  (letrec-exp
    (p-names (list-of identifier?))
    (paramss (list-of (list-of identifier?)))
      ...
      (procedure (params body saved-env)
        (value-of
          ...
          (let f ([ps params] [as args])
            (if (null? ps)
              saved-env
              (extend-env (car ps) (car as) (f (cdr ps) (cdr as)))
            )
          )
        )
      )
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (let-exp (param exp1 body)
        (value-of
          body
          (extend-env param (value-of exp1 env) env)
        )
      )
      (letrec-exp (p-names paramss p-bodys letrec-body)
        (value-of
          letrec-body
          (extend-env-rec p-names paramss p-bodys env)
        )
      )
      (minus-exp (exp1)
          ...
          (
            ...
            [
              ...
              (let f ([rs rands])
                (if (null? rs)
                  '()
                  (cons (value-of (car rs) env) (f (cdr rs)))
                )
              )
            ]
          )
        ...
        (list
          (if-exp (zero?-exp (var-exp 'y))
            ...
            (diff-exp
              (call-exp
                (var-exp 'g)
                (list (var-exp 'x) (diff-exp (var-exp 'y) (const-exp 1)))
              )
              (minus-exp
                (call-exp
                  (var-exp 'h)
